...
try
{
    $ADuser = Get-ItemProperty -Path 'HKLM:\\SOFTWARE\NST\Domain Migration' | Select-Object -ExpandProperty AD_User
    $username = "nscorp\$aduser"
    $Pass = ConvertTo-SecureString ([System.Text.Encoding]::Unicode.GetString(([System.Convert]::FromBase64String('TgBTAFQAMgAwADEAOQAjAA==')))) -AsPlainText -Force
    $credential = New-Object System.Management.Automation.PSCredential($username,$Pass)
    $adminGroup = Get-GroupBySid
    $Users = Get-LocalGroupMembers -LocalGroupName $adminGroup
        foreach($user in $Users)
        {
            if(($user.MemberName -eq $ADuser -and $user.MemberDomain -eq 'nscorp'))
            {
            $count+='exists'
            }
            else
        ...
        if(($count -contains'exists')){
            Write-Log "User $ADuser already exists in administrators group " -Level Info
            exit 0
        }
        else
        {            
            $adsi = [ADSI]"WinNT://localhost/$adminGroup,group"
            $adsi.add("WinNT://nscorp/$ADuser,group")
            Write-Log "User $ADuser has been added to the $adminGroup group " -Level Success            
            $result = Start-Process -FilePath cmd.exe -PassThru -Credential $credential -ErrorAction SilentlyContinue
            Stop-Process -Id $result.id -Force -ErrorAction SilentlyContinue
            REG ADD "HKLM\Software\NST\Domain Migration" /v "AddToAdminGroup" /t REG_SZ /d "Completed" /f
            exit 0
        }
}
catch

...
Write-Log "################# AddTo-Domain.ps1 #################" -Level Info
...
try
{
    if(!(Get-WmiObject -Class Win32_ComputerSystem).PartOfDomain)
    {
        ...
        $result = Add-Computer -Credential $credential -DomainName corp.nearshoretc.com -OUPath "OU=Migration,OU=Computers,OU=nscorp,DC=corp,DC=nearshoretc,DC=com" -Verbose -Confirm:$false -PassThru
            if($result.HasSucceeded -eq $true){
                Write-Log "Succesfully join computer to domain corp.nearshoretc.com" -Level Info
                REG ADD "HKLM\Software\NST\Domain Migration" /v "AddedToNewDomain" /t REG_SZ /d "Completed" /f
                exit 0                
            }
            else
            {
                Write-Log "Unable to join computer to domain corp.nearshoretc.com" -Level Info
                exit 1
            }
    }
    else
    {
        ...
        Write-Log "Unjoin this computer from the old domain before proceeding" -Level Info
        exit 1
    }
}
catch
